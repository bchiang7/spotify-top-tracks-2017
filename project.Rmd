...
library(rpart)
# library(rpart.plot)
top2017$rank <- c(1:100)
tree_model <- rpart(rank ~ ., data = top2017)
plot(tree_model, box.palette = "GnBu")
```
...
pop <- dbSendQuery(db, "SELECT 
                        CASE
                          WHEN track_popularity < 49 then '0 - 49'
                          WHEN track_popularity between 50 and 59 then '50 - 59'
                          WHEN track_popularity between 60 and 69 then '60 - 69'
                          WHEN track_popularity between 70 and 79 then '70 - 79'
                          WHEN track_popularity between 80 and 89 then '80 - 89'
                          WHEN track_popularity between 90 and 100 then '90 - 100'
                          ELSE 'other'
                        END as `range`, COUNT(*)
                        FROM top2017 
                        GROUP BY `range`;")
...
smp_size <- floor(0.5 * nrow(dataNumeric))

# set the seed to make your partition reproductible
set.seed(123)
train_ind <- sample(seq_len(nrow(dataNumeric)), size = smp_size)

# training data set
train_data <- dataNumeric[train_ind, ]

# validation data set (all values not in the training data set)
test_data <- dataNumeric[-train_ind, ]
```
...
Formula for multiple linear regression:  “YVAR ~ XVAR1 + XVAR2 + … + XVARi” where YVAR is the dependent, or predicted, variable and XVAR1, XVAR2, etc. are the independent, or predictor, variables.

### predict the track popularity using all other numeric variables

```{r}
all_model <- lm(formula = track_popularity ~ danceability + energy + loudness + speechiness + acousticness + instrumentalness + liveness + valence + tempo + duration, data = train_data)
summary(all_model)

pred <- predict(all_model, train_data)
plot(pred - (train_data$track_popularity))
```

With an intercept of 63.2, the regression equation is:

```
Popularity = (11.04 * danceability) + (13.04 * energy) + (11.4 * loudness) + (-0.04 * speechiness) + (-11.0 * acousticness) + (-41.3 * instrumentalness) + (4.35 * liveness) + (-1.67 * valence) + (-1.54 * tempo) + (-1.96 * duration)
```

Make a prediction with the model above

```{r}
predict(all_model, data.frame(danceability = 0.9, energy = 0.7, loudness = 0.3, speechiness = 0.1, acousticness = 0.1, instrumentalness = 0.00003, liveness = 0.02, valence = 0.5, tempo = 0.65, duration = 0.5))
```

Based on this model, the popularity of a track with features where danceability is 0.9, energy is 0.7, loudness is 0.3, speechiness is 0.1, acousticness is 0.1, instrumentalness is 0.00003, liveness is 0.02, valence is 0.5, tempo is 0.65, and duration is 0.5 will have a popularity of approximately 81.

<br>

### Predict the track popularity using danceability, energy, liveness, and tempo

```{r}
delt_model <- lm(formula = track_popularity ~ danceability + energy + liveness + tempo, data = train_data)
summary(delt_model)

pred <- predict(delt_model, train_data)
plot(pred - (train_data$track_popularity))
```
...
```{r}
predict(delt_model, data.frame(danceability = 0.5, energy = 0.9, liveness = 0.02, tempo = 0.8))
```

Based on this model, the popularity of a track with features where danceability is 0.5, energy is 0.9, liveness is 0.02, and tempo is 0.8 will have a popularity of approximately 75.
