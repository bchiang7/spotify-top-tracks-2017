...
all_model <- lm(formula = track_popularity ~ danceability + energy + loudness + speechiness + acousticness + instrumentalness + liveness + valence + tempo + duration + time_signature, data = train_data)
summary(model)
...
With an intercept of 50.7, the regression equation is:

```
Popularity = (10.6 * danceability) + (13.6 * energy) + (1.3 * loudness) + (-1.2 * speechiness) + (-11.7 * acousticness) + (-44.9 * instrumentalness) + (2.55 * liveness) + (-1.2 * speechiness) + (-1.2 * speechiness) + 

```

```{r}
model <- rpart(formula = track_popularity ~ danceability + energy + loudness + speechiness + acousticness + instrumentalness + liveness + valence + tempo + time_signature, data = train_data)
summary(model)
...
```{r}
# predict the track popularity using danceability, energy, tempo
model <- lm(formula = track_popularity ~ danceability + energy + liveness + tempo, data = train_data)
summary(model)
...
predict(model, data.frame(danceability = 0.8, energy = 0.9, liveness = 0.1, tempo = 120, instrumentalness = 0000.1))
```
